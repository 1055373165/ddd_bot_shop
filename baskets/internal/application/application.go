...
type (
	...
	App interface {
		...
		GetBaskets(ctx context.Context) ([]*domain.Basket, error)
	}
...
func New(baskets domain.BasketRepository, stores domain.StoreRepository, product domain.ProductRepository, orders domain.OrderRepository) *Application {
	return &Application{
		baskets:  baskets,
		stores:   stores,
		products: product,
...
func (a Application) GetBaskets(ctx context.Context) ([]*domain.Basket, error) {
	return a.baskets.FindAll(ctx)
}
