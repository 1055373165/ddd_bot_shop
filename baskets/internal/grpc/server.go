...
func (s server) GetBaskets(ctx context.Context, request *basketspb.GetBasketsRequest) (*basketspb.GetBasketsResponse,
	error,
) {
	domainBaskets, err := s.app.GetBaskets(ctx)
	if err != nil {
		return nil, err
	}

	pbBaskets := []*basketspb.Basket{}

	for _, domainBasket := range domainBaskets {
		pbBaskets = append(pbBaskets, s.basketFromDomain(domainBasket))
	}

	return &basketspb.GetBasketsResponse{
		Baskets: pbBaskets,
	}, nil
}
