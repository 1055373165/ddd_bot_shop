...
func (s server) GetOrders(ctx context.Context, request *orderingpb.GetOrdersRequest) (*orderingpb.GetOrdersResponse, error) {
	orders, err := s.app.GetOrders(ctx, queries.GetOrders{})
	if err != nil {
		return nil, err
	}

	pbOrders := make([]*orderingpb.Order, len(orders))
	for _, order := range orders {
		pbOrders = append(pbOrders, s.orderFromDomain(order))
	}

	return &orderingpb.GetOrdersResponse{Orders: pbOrders}, nil
}
