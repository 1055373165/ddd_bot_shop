...
func (r OrderRepository) FindAll(ctx context.Context) ([]*domain.Order, error) {
	const query = "SELECT id, customer_id, payment_id, shopping_id, invoice_id, items, status FROM %s"
	...
	defer func(rows *sql.Rows) {
		if err := rows.Close(); err != nil {
			err = errors.Wrap(err, "closing order rows")
		}
	}(rows)
	...
	for rows.Next() {
		...
		err = rows.Scan(&order.ID, &order.CustomerID, &order.PaymentID, &order.ShoppingID, &order.InvoiceID, &items, &status)
		if err != nil {
			return nil, err
		}
		order.Status = domain.ToOrderStatus(status)
